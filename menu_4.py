...
class NewMenuDemo(Frame): # расширенный фрейм
    ...
    def makeToolBar(self, size=(40,40)):    # тулбар
        from PIL.ImageTk import PhotoImage, Image  # для jpeg или новых миниатюр
        imgdir = r'C:/Users/Ivar/Pictures/Скрины/'
        toolbar = Frame(self, cursor='hand2', relief = SUNKEN, bd = 2)
        toolbar.pack(side=BOTTOM, fill=X)
        photos = 'index1.jpg', 'index2.jpg', 'q1.gif'
        self.toolPhotoObjs = []
        for file in photos:
            imgobj = Image.open(imgdir + file)  # создать новую миниатюру
            imgobj.thumbnail(size, Image.ANTIALIAS)  # подгоняет фотку под размер
    ...
    def imageMenu(self):
        photoFiles = ('q1.gif', 'q2.gif', 'q3.gif')
        pulldown = Menu(self.menubar)
        self.photoObjs = []
        for file in photoFiles:
            img = PhotoImage(file='C:/Users/Ivar/Pictures/Скрины/' + file)
