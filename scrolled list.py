# Listbox - списки элементов доступных для выбора
# Scrollbar - прокрутка содержимого других виджетов
...
class ScrolledList(Frame):
    ...
    def handleList(self, event):
        index = self.listbox.curselection()  # индекс элемента
        label = self.listbox.get(index)  # извлечь выбранный текст
        her = self.listbox.get('active')  # заменяет две строчки выше
        self.runCommand(label, her)  # и вызвать действие
        # или get(ACTIVE)
    ...
    def runCommand(self, *selection):  # необходимо переопределить
        print('You selected:', selection[0])
        print('хер нажал -->', selection[1])
