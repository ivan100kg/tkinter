...
# создается отдельное окно Toplevel с необходимыми виджетами
# фокус ввода передается на окно, а остальные окна становятся неактивными и
# ожидать события

import sys
from tkinter import *

makemodal = (len(sys.argv) > 1) # есть ли аргументы командной строки
...
def dialog():
    ...
    if makemodal:
        win.focus_set() # принять фокус ввода, применяется к виджетам == focus()
