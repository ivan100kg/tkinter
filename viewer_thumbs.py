...
def makeThumbs(imgdir, size=(100, 100), subdir='thumbs'):
    ...
    thumbdir = os.path.join(imgdir, subdir) # адрес папки thumbs в нашей папке
    if not os.path.exists(thumbdir):    # создаем папку thumbs если нету
        os.mkdir(thumbdir)
...
def viewer(imgdir, kind=Toplevel, cols=None):   # принимает адрес
    """
...
if __name__ == '__main__':
    # путь к папке с фотками или через ком строку
    imgdir = (len(sys.argv) > 1 and sys.argv[1]) or r'C:\Users\Ivar\Pictures\Скрины'
    main, save = viewer(imgdir, kind=Tk)    # возвращает Tk() и список
    main.mainloop()